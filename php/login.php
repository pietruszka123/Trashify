...
if (isset($_COOKIE['token']) &&  $_COOKIE['token'] != "notSet" && isTokenValid($_COOKIE['token']) && $_SERVER['REQUEST_URI'] == 'login.php') { //If the token is valid, then sends you to the index page (if already not somewhere else than login page)
    //header("Location: index.php");
} else if (isset($_COOKIE['token']) &&  $_COOKIE['token'] != "notSet" && isTokenValid($_COOKIE['token'])) {
    ;
} else if ($_SERVER['REQUEST_URI'] != 'login.php') { //If you don't have a cookie token then sets it to "notSet" and sends you to the login page if you're not there
...
if (emailCorrect($_POST['email']) && passwordCorrect($_POST['password'])) { //Checks if the input is correct
$sql = "SELECT `users`.`id`, `users`.`password` FROM `users` WHERE `users`.`email` = ?";

if ($stmt = $mysqli->prepare($sql)) { //Searches for the user id and password
...
if ($row['password'] == $_POST['password']) { //Checks if the password given by the user is correct
$userId = $row['id'];
...
$row = $result->fetch_assoc();
                            ...
                            if (isset($row['token']) && $row['token'] != NULL) { //Checks if the the token is still valid and logs in the user
  ...
  <div class="login-box">
    ...
    <form method="POST">
      ...
      <div class="loginInputBox">
        <input type="email" id="email" name="email" class="loginInput" placeholder="Email"><br>
        <input type="password" id="password" name="password" class="loginInput" placeholder="Password" pattern="^[a-zA-Z0-9]{3,64}$"><br>
      </div>

      <div class="loginRememberAndSubmit">
        <div class="loginRememberMeBox">
          <label for="loginRemember" class="loginRememberMe">Remember me:
            <input type="checkbox" id="rememberMe" name="rememberMe" class="loginRememberMeCheckBox">
          </label>
        </div>
        <input type="submit" id="submit" name="submit" class="loginSubmit" value="Log in">
      </div>
